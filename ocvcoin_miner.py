...
def block_mine(block_template, address, pool_address, stlxaddress, cpu_index, cpuCount, event):       
    ...
    nonce_start = (nonce_start - (nonce_start % 10000))
    
    
    nonce_end = ((cpu_index+1) * int(0xffffffff / cpuCount))
    
    nonce_end = (nonce_end - (nonce_end % 10000))    
...
def standalone_miner(address, pool_address, stlx_address):
            ...
            if re_init_arr:
                ...
                for i in range(process_count): 
                    if block_template["curtime"] >= 1636416000: #GMT 	Tue Nov 09 2021 00:00:00 GMT+0000
                        ...
                        proc_arr.insert(i, multiprocessing.Process(target=block_mine, args=(block_template, address, pool_address, stlx_address, i,cpuCount,event)))
                    proc_arr[i].start()
            time.sleep(0.35)
...
''')
    ...
    standalone_miner(addr, paddr, stlxaddr)
